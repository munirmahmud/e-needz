...
const ModuleProductHasVariants = ({ ecomerce, currency, product }) => {
  ...
  function handleSelectColor(e, colorId) {
    if (product && product.variants.length > 0) {
      const existVariant = product.variants.find((item) => item.id === colorId);
    ...
    if (selectedVariant.is_sale) {
      priceArea = (
        <h4 className="ps-product__price sale">
          <del className="mr-2" style={{ color: "#aeaeae" }}>
        ...
        return (
          <div
            className={`ps-variant ps-variant--image ${
              selectedVariant && selectedVariant.id === item.id ? "active" : ""
            }`}
            key={item.id}
          ...
          >
            ...
            {item.thumbnail !== null ? (
              <img src={`${baseUrl}${item.thumbnail.url}`} alt="" />
            ) : (
              ""
            )}
          </div>
          ...
          return (
            <div
              className={`ps-variant ps-variant--size ${
                selectedSize && selectedSize.id === item.id ? "active" : ""
              }`}
              key={item.id}
              onClick={(e) => handleSelectSize(e, item.id)}
            >
              <span className="ps-variant__tooltip">{item.name}</span>
              <span className="ps-variant__size">{item.character}</span>
            </div>
          );
        });
      }
          ...
          <figure>
            <figcaption>
              ...
              <strong className="pl-1">
                {selectedSize !== null ? selectedSize.name : "Choose an option"}
              </strong>
            </figcaption>
            {sizeSelectionArea}
          </figure>
